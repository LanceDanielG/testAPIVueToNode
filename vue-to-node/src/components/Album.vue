        ...
        <table class="border-collapse text-sm table-auto">
          ...
          <tbody class="bg-white dark:bg-slate-800">
            <tr>
                ...
                <td class="px-2 py-1">
                    <input type="text" v-model="album" name="album" id="album" class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Album Name">
                    <input type="hidden" v-model="uid" name="uid" id="uid" placeholder="ID">
                </td>
...
<script>
    ...
    export default {
        ...
        mounted(){
            if (this.albumById) {
                this.artist = this.albumById.artist;
                this.album = this.albumById.album;
                this.uid = this.albumById.id;
            }
        },
        watch: {
          albumById: {
        ...
        methods: {
            ...
            async storeAlbums(){
                try {
                    ...
                    // });
                    axios
                        .post('http://localhost:8080/album/storeAlbum', {artist: this.artist, album: this.album, id: this.uid})
                        .then((res) => {
                            this.fetchAlbums();
                        })
                } catch (error) {
            ...
            async getAlbumByID(id) {
                axios.get(`http://localhost:8080/album/getAlbumById/${id}`)
                    .then((res) => {
            ...
            async deleteAlbum(id){
                axios
                    .delete(`http://localhost:8080/album/deleteAlbum/${id}`)
                    .then(res => {
                        this.fetchAlbums(); 
                    })
                    .catch(error => {
